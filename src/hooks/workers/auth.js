...
const connect = async (url, user, login) => {
  ...
  _ws.onmessage = (msg) => {
    postMessage({ skip: false, type: login ? "login" : "register", result: { ...JSON.parse(msg.data), mnemonic }  })
    _ws.close()
  }
...
const createKey = async (data) => {
  ...
  const mnemonic = entropyToMnemonic(Buffer.from(randomBytes(16)))
  const node = fromSeed(mnemonicToSeedSync(mnemonic))
  ...
  const { privateKey, compressed } = wif.decode(nodePath.toWIF(), 128)
  const signature = encrypt(privateKey, compressed, data.password)
