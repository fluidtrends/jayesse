...
export const useData = () => {
  ...
  const [authWorker, setAuthWorker] = useState(new Worker('./workers/auth.js', { type: 'module' }))
  const [results, setResults] = useState<any>({})

  const getDoc = async (hash: string) => {
    const chunks = []
    for await (const data of node.cat(hash)) {
      chunks.push(data)
    }
    return Buffer.concat(chunks).toString()
  }

  const getJSONDoc = async (hash: string) => {
    const doc = await getDoc(hash)
    return JSON.parse(doc)
  }

  const addDoc = async (data: string) => {
    const { cid } = await node.add(data)
    return cid.toString()
  }

  const addJSONDoc = async (data: any) => {
    return addDoc(JSON.stringify(data))
  }

  authWorker.onmessage = async (event: any) => {
    const { data } = event
    const { type, skip, result } = data

    if (skip || !result) {
      setResults({ ...results, [type]: { ...results[type], ...data, working: true }})
      return 
    }
    ...
    switch (type) {
      ...
      case "register":
        ...
        return
      case "login":
        const user2 =  { ...data.result.data.user, _hash: data.result.hash }
        put("user", user2)
        setUser(user2)
        setResults({ ...results, [type]: { ...results[type], result: { user: user2 }, working: false }})
        setWorking(false)
        return
    }

    setResults({ ...results, [type]: { ...results[type], ...data, working: false }})
    setWorking(false)
  }
  ...
  const login = async (username: string, password: string) => {
    ...
    const profile = {
      ...results.findUser.result.user,
      details: JSON.parse(results.findUser.result.user.details)
    }
  ...
  const findUser = async (username: string, login: boolean) => {
    setWorking(true)
    const id = "3"//nanoid()
    setResults({ ...results, findUser: { id, working: true, username, login }})
  ...
  const  addSignedObject = async (payload: any) => {
